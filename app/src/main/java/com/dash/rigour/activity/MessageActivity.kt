...
import com.dash.rigour.data.Chat
import com.dash.rigour.data.NotificationData
import com.dash.rigour.data.PushNotification
import com.dash.rigour.data.RetrofitInstance
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        reference!!.addValueEventListener(object : ValueEventListener {
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                val user =
                    snapshot.getValue(User::class.java)/*   binding.tvUserName.text = user!!.userName*/
                if (user?.userImage == "") {
                    binding.imgProfile.setImageResource(R.drawable.profile)
                } else {
                    Glide.with(this@MessageActivity).load(user?.userImage).into(binding.imgProfile)
                }
            }
        })
        ...
        binding.btnSendMessage.setOnClickListener {
            ...
            if (message.isEmpty()) {
                ...
                topic = "/topics/$userId"
                PushNotification(
                    NotificationData(userName!!, message), topic
                ).also {
                    sendNotification(it)
                }
